...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/11/30 20:05:40 by mfrancis          #+#    #+#             */
/*   Updated: 2024/11/30 20:05:40 by mfrancis         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*    
            ...
            [] ler input  - readline ?
				...
				[] Parser
					[] BNF (apenas um doc para mim p conseguir fazer o sintax error)
					'--> [] Error sintax
							'--> [] AST
                [/] execution
                    [/] build-in or system ?
                       [x]  pwd (Finito, e sem memory leaks e sem erros de norma)
                       [x]  cd (Finito, e sem memory leaks e sem erros de norma)
							[x] Corrigir o bug, quando estou no dir '/' e ando para outro diretorio, o output é '//home'
                       [x]  env (Finito, e sem memory leaks e sem erros de norma)
                       [x]  export (Finito, e sem memory leaks e sem erros de norma)
							[x] Não pode aceitar variaveis com numeros 
							[x] Dar export de local variables

                       [/]  echo -n (Sem memory leaks, norma falta o header. Falta considerar algumas condicoes)
                       [x]  unset (Finito, e sem memory leaks e sem erros de norma)
                       [x]  exit (Finito, fiz o reaproveitamento da funcao que ja existe)
			[x] free(input);
			[x] Executor;
				[x] Documentacao (Finito)
				[x] Aceitar os comandos a partir do diretorio, por exemplo /bin/ls
			[x] CORRIGIR PIPEX;
				[x] Caso nao exista o infile, mas ja existe informacao no outfile, tenho de limpar quando ex (infile ls wc outfile)
				[] Adicionar a funcionalidade "here_doc"
				[] Caso o numero de argumentos esteja errado no here_doc, a mensagem de erro aparece dentro do outfile
			[] LOCAL VARIABLES
				...
				[] Caso faça teste=! e dps eu fizer export teste=1, tenho de tirar da local_var e exportar com o novo valor 
				OUTPUTS ESTRANHOS
					[] test= | A variavel fica com o valor nulo
					[x] unset teste | A função unset tb funciona para variaveis locais
