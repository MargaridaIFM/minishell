...
// 	{
// 		if (ast->token->cmd)
// 			printf("(%d: %s file associated %s  comand %s dq %d)\n", ast->token->type, ast->token->str, ast->token->path, ast->token->cmd[0], ast->token->dq);
// 		else
// 			printf("(%d: %s file associated %s dq %d)\n", ast->token->type, ast->token->str, ast->token->path, ast->token->dq);
// 	}
// 	else
// 		printf("(%d: %s file associated %s dq %d)\n", ast->token->type, ast->token->str, ast->token->path, ast->token->dq);
// 	if (ast->left != NULL)
